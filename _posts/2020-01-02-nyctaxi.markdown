...
out entirely in parallel.  The vintage /machine/ we are using extracts
heatmap data at a rate of about 5000 heatmaps per second.  Drawing the
heatmaps using /pillow/ is significantly slower, though.  But will
full parallelisation, the machine can draw about 125 frames per
second.  At 60Hz framerate, this is twice playback realtime.  Then,
/ffpmeg/ needs about the same time to encode the frames to h.264 video
format.
